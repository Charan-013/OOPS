...
class ArrayListADT(Generic[T]):
    ...
    def _resize(self, new_capacity: int) -> None:
        resizeList: List[Optional[T]] = [None] * new_capacity
        for i in range(self._size):
            resizeList[i] = self._data[i]
        self._data = resizeList
    ...
    def rearrange_list(self):
        for i in range(len(self._data)):
            for j in range(len(self._data)):
                if self._data[i] == None and self._data[j] != None and i < j:
